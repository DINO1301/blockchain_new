...
import PreBlockchainReview from './pages/admin/PreBlockchainReview';
    ...
    <div className="flex flex-col min-h-screen bg-brand-lightBlue font-sans text-brand-navy" style={{ minHeight: '100vh' }}>
      <header className="bg-white/80 backdrop-blur-md shadow-sm sticky top-0 z-50 border-b border-blue-50">
        <div className="max-w-[1400px] mx-auto px-4 sm:px-6 h-16 flex items-center justify-between gap-4">
          ...
          <div className="hidden lg:flex items-center gap-2 xl:gap-5 py-2">
            ...
            {user && (role === "admin" || role === "admin_on_chain") && (
              <div className="flex items-center gap-2 xl:gap-5 pl-3 xl:pl-5 border-l border-gray-200">
                ...
                <Link to="/admin/review" className={navClass('/admin/review')}>Kiểm duyệt</Link>
                ...
                <>
                  <div className="text-right hidden xl:block leading-tight max-w-[120px]">
                    ...
                    <div className="flex items-center gap-1 justify-end mt-0.5">
                      {role === 'admin_on_chain' && <span className="text-[8px] font-black text-white bg-emerald-500 px-1 rounded uppercase">On-chain</span>}
          ...
          <div className="lg:hidden absolute top-16 left-0 w-full bg-white shadow-xl border-t border-gray-100 flex flex-col z-40 max-h-[calc(100vh-4rem)] overflow-y-auto">
            ...
            {user && (role === "admin" || role === "admin_on_chain") && (
              <div className="p-4 flex flex-col gap-1 bg-gray-50 border-b border-gray-100">
                ...
                <Link to="/admin/review" className={mobileNavClass('/admin/review')} onClick={closeMenu}>Kiểm duyệt</Link>
      ...
      <main className="flex-grow pt-10 pb-24 min-h-[70vh]">
        <Routes>
          ...
          <Route path="/admin/dashboard" element={<ProtectedRoute allowedRoles={['admin', 'admin_on_chain']}><AdminDashboard /></ProtectedRoute>} />
          <Route path="/admin/inventory" element={<ProtectedRoute allowedRoles={['admin', 'lab', 'admin_on_chain']}><Inventory /></ProtectedRoute>} />
          <Route path="/admin/review" element={<ProtectedRoute allowedRoles={['admin', 'lab', 'admin_on_chain']}><PreBlockchainReview /></ProtectedRoute>} />
          <Route path="/profile" element={<ProtectedRoute allowedRoles={['user', 'admin', 'lab', 'admin_on_chain']}><Profile /></ProtectedRoute>} />
          <Route path="/admin/products" element={<ProtectedRoute allowedRoles={['admin', 'admin_on_chain']}><ProductList /></ProtectedRoute>} />
          <Route path="/admin/products/new" element={<ProtectedRoute allowedRoles={['admin', 'admin_on_chain']}><ProductManager /></ProtectedRoute>} />
          <Route path="/admin/users" element={<ProtectedRoute allowedRoles={['admin', 'admin_on_chain']}><UserManager /></ProtectedRoute>} />
          <Route path="/cart" element={<ProtectedRoute allowedRoles={['user', 'admin', 'lab', 'admin_on_chain']}><Cart /></ProtectedRoute>} />
          <Route path="/orders" element={<ProtectedRoute allowedRoles={['user', 'admin', 'lab', 'admin_on_chain']}><OrderHistory /></ProtectedRoute>} />
        </Routes>
      </main>
