...
import { Search, Package, MapPin, Clock, User, CheckCircle2, Loader2, FileText, Image as LucideImage, ExternalLink, QrCode, Map as MapIcon } from 'lucide-react';
          ...
          <div className="lg:col-span-4">
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 lg:sticky lg:top-24">
              <div className="flex flex-col items-center text-center mb-6">
                <div className="relative mb-6">
                  {productDetails?.image_url ? (
                    <img 
                      ...
                      alt={batchData.name}
                      className="w-24 h-24 sm:w-32 sm:h-32 object-contain bg-gray-50 rounded-3xl p-4 shadow-inner"
                    />
                  ) : (
                    <div className="w-20 h-20 sm:w-24 sm:h-24 bg-blue-50 rounded-3xl flex items-center justify-center text-primary">
                      <Package size={40} />
                    </div>
                  )}
                ...
                <h2 className="text-lg sm:text-xl font-black text-gray-800 leading-tight mb-2">{batchData.name}</h2>
                <p className="text-[10px] font-medium text-gray-400 uppercase tracking-widest">{batchData.manufacturer}</p>
              </div>
              
              <div className="space-y-3 pt-4 border-t border-gray-50">
                <div className="p-3 bg-gray-50 rounded-xl border border-gray-100">
                  <p className="text-[9px] text-gray-400 uppercase font-black mb-1 tracking-wider">Sở hữu hiện tại</p>
                ...
                <div className="flex items-center justify-between p-3 bg-green-50 rounded-xl border border-green-100">
                  <div>
                    <p className="text-[9px] text-green-600 uppercase font-black mb-1 tracking-wider">Trạng thái lô</p>
                    <span className="text-xs font-bold text-green-700">
                      {batchData.status === 0 ? "Đã sản xuất" : batchData.status === 1 ? "Đã kiểm định" : batchData.status === 2 ? "Đang vận chuyển" : "Đã giao hàng"}
                    </span>
                  </div>
                  <div className="bg-white p-1.5 rounded-lg text-green-500 shadow-sm">
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden min-h-[500px]">
              ...
              <div className="flex p-2 bg-gray-50/50 gap-2">
                <button 
                  ...
                  className={`flex-1 py-4 rounded-xl text-[10px] sm:text-xs font-black flex flex-col sm:flex-row items-center justify-center gap-2 transition-all ${activeTab === 'timeline' ? 'bg-white text-primary shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  <Clock size={16} /> <span className="uppercase">Hành trình</span>
                </button>
                <button 
                  onClick={() => setActiveTab('docs')}
                  className={`flex-1 py-4 rounded-xl text-[10px] sm:text-xs font-black flex flex-col sm:flex-row items-center justify-center gap-2 transition-all ${activeTab === 'docs' ? 'bg-white text-indigo-600 shadow-sm' : 'text-gray-400 hover:text-gray-600'}`}
                >
                  <FileText size={16} /> <span className="uppercase">Giấy tờ</span>
                </button>
              </div>
              ...
              <div className="p-4 sm:p-6">
                {activeTab === 'timeline' ? (
                  /* --- PHẦN HÀNH TRÌNH --- */
                  <div className="space-y-8 relative before:absolute before:inset-0 before:left-4 sm:before:left-5 before:w-0.5 before:bg-gradient-to-b before:from-blue-100 before:to-transparent">
                    {timeline.length > 0 ? (
                      [...timeline].reverse().map((event, index) => (
                        <div key={index} className="relative pl-8 sm:pl-10 group">
                          <div className={`absolute left-0 sm:left-1 w-8 h-8 rounded-xl border-4 border-white flex items-center justify-center shadow-md z-10 transition-transform group-hover:scale-110 ${index === 0 ? 'bg-primary text-white ring-4 ring-blue-50' : 'bg-white text-gray-300 border-gray-100'}`}>
                            {index === 0 ? <Package size={14}/> : <MapPin size={14}/>}
                          </div>
                          <div className="bg-white rounded-xl p-4 border border-gray-100 hover:border-blue-100 hover:shadow-lg hover:shadow-blue-50/50 transition-all duration-300">
                            <div className="flex flex-col mb-3">
                              <h3 className="font-black text-gray-800 text-xs sm:text-sm leading-tight mb-1">
                                {event.description}
                              </h3>
                            ...
                            <div className="flex items-start gap-2 bg-gray-50/80 p-2 rounded-lg border border-gray-50">
                      ...
                      <div className="text-center py-20">
                         ...
                         <p className="text-gray-400 font-bold uppercase text-[10px] tracking-widest">Chưa có lịch sử</p>
                      </div>
                    )}
                  </div>
                ) : activeTab === 'docs' ? (
                  ...
                  <div className="space-y-4 animate-in fade-in duration-500">
                    {docs.length > 0 ? (
                      <div className="grid grid-cols-1 gap-4">
                        {docs.map((url, index) => (
                          <div key={index} className="group bg-white rounded-2xl border border-gray-100 overflow-hidden hover:shadow-xl hover:shadow-indigo-100 transition-all duration-500">
                            ...
                            <div className="p-3 flex justify-between items-center">
                              <span className="text-xs font-black text-gray-800">Ảnh hồ sơ #{index + 1}</span>
                              <a 
                                href={url} 
                                target="_blank" 
                                rel="noreferrer" 
                                className="w-8 h-8 bg-indigo-50 text-indigo-600 rounded-lg flex items-center justify-center hover:bg-indigo-600 hover:text-white transition-colors"
                              >
                      ...
                      <div className="text-center py-20 bg-gray-50 rounded-[30px] border-4 border-dashed border-gray-100">
                        <LucideImage size={48} className="mx-auto text-gray-200 mb-4" />
                        <p className="text-xs text-gray-500 font-medium">Chưa cập nhật giấy tờ pháp lý.</p>
                      </div>
                    )}
                  </div>
                ) : null}
