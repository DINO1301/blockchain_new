...
const Shop = () => {
  ...
  const uniqueBrands = useMemo(() => {
    ...
    const defaults = ['Stella Pharm', 'Abbott', 'Hasan', 'Japanwell', 'DHG Pharma', 'Traphaco'];
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(p => {
      ...
      const matchCountry = selectedCountries.length === 0 || selectedCountries.includes(p.country || 'Việt Nam');

      // Lọc theo thương hiệu
      const matchBrand = selectedBrands.length === 0 || selectedBrands.includes(p.brand || 'Khác');
