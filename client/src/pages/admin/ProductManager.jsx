...
import { 
  ...
  AlertTriangle, Thermometer, FileSearch, X, QrCode, Upload, Filter, GripVertical, 
  Globe, Building2
} from 'lucide-react';
...
const UploadSection = ({ title, field, value, icon: IconEl, colorClass, uploading, handleFileUpload, setFormData, formData, reorderConfig }) => {
  ...
  const removeFile = (field, index) => {
    ...
    if (reorderConfig) {
      const arr = urls.filter((_, i) => i !== index);
      reorderConfig.setUrls(arr);
      if (reorderConfig.onSync) reorderConfig.onSync(arr);
    } else {
  ...
  return (
    <div className="space-y-4 bg-white p-5 rounded-2xl border border-gray-100 shadow-sm">
      ...
      {/* Danh sách file đã thêm (gọn, hỗ trợ kéo thả khi có reorderConfig) */}
      {urls.length > 0 ? (
...
const ProductManager = () => {
  ...
  const handleFileUpload = async (event, field) => {
    ...
    try {
      ...
      setFormData(prev => ({ ...prev, [targetField]: prev[targetField] ? `${prev[targetField]}, ${joinedUrls}` : joinedUrls }));

      if (field === 'main') {
        const prevArr = [
          ...(formData.imageUrl ? formData.imageUrl.split(',').map(u=>u.trim()).filter(Boolean) : []),
          ...(formData.imageUrlsText ? formData.imageUrlsText.split(',').map(u=>u.trim()).filter(Boolean) : [])
        ];
        const newArr = [...prevArr, ...publicUrls];
        setAllImages(newArr);
        setFormData(prev => ({ ...prev, imageUrl: newArr[0] || '', imageUrlsText: newArr.slice(1).join(', ') }));
      } else if (field === 'docs') {
        const prevArr = (formData.relatedDocsText || '').split(',').map(u=>u.trim()).filter(Boolean);
        const newArr = [...prevArr, ...publicUrls];
        setAllDocs(newArr);
        setFormData(prev => ({ ...prev, relatedDocsText: newArr.join(', ') }));
      } else if (field === 'qrs') {
        const prevArr = (formData.relatedQRsText || '').split(',').map(u=>u.trim()).filter(Boolean);
        const newArr = [...prevArr, ...publicUrls];
        setAllQrs(newArr);
        setFormData(prev => ({ ...prev, relatedQRsText: newArr.join(', ') }));
      }
  ...
  return (
    <div className="max-w-[90rem] mx-auto p-6">
      <div className="bg-white rounded-xl shadow-lg border border-gray-100 p-8">
        ...
        <form onSubmit={handleSubmit} className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px] gap-10">
          ...
          <div className="space-y-6">
            <UploadSection 
              ...
              reorderConfig={{
                ...
                setUrls: (arr) => {
                  setAllImages(arr);
                  setFormData(prev => ({ ...prev, imageUrl: arr[0] || '', imageUrlsText: arr.slice(1).join(', ') }));
                },
                onSync: (arr) => setFormData(prev => ({ ...prev, imageUrl: arr[0] || '', imageUrlsText: arr.slice(1).join(', ') }))
              }}
            />
            ...
            {/* Preview Ảnh chính */}
                  <div className="bg-white border border-gray-100 rounded-2xl h-64 flex items-center justify-center overflow-hidden shadow-sm relative group">
                    {formData.imageUrl ? (
                      <img 
                        src={(Array.isArray(formData.imageUrl) ? (formData.imageUrl[0] || '') : String(formData.imageUrl || '').split(',')[0]).trim()} 
                        alt="Preview" 
            ...
            <UploadSection 
              ...
              reorderConfig={{
                ...
                setUrls: (arr) => {
                  setAllDocs(arr);
                  setFormData(prev => ({ ...prev, relatedDocsText: arr.join(', ') }));
                },
                onSync: (arr) => setFormData(prev => ({ ...prev, relatedDocsText: arr.join(', ') }))
              }}
            />
            ...
            <UploadSection 
              ...
              reorderConfig={{
                ...
                setUrls: (arr) => {
                  setAllQrs(arr);
                  setFormData(prev => ({ ...prev, relatedQRsText: arr.join(', ') }));
                },
                onSync: (arr) => setFormData(prev => ({ ...prev, relatedQRsText: arr.join(', ') }))
              }}
            />
