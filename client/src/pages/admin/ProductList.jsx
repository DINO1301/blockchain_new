...
const COUNTRY_OPTIONS = [
  'Hoa Kỳ', 'Hàn Quốc', 'Nhật Bản', 'Pháp', 'Việt Nam', 'Úc', 'Đức', 'Ấn Độ', 'Thái Lan', 'Malaysia', 'Singapore', 'Indonesia', 'Đài Loan', 'Hồng Kông'
];

const BRAND_OPTIONS = [
  'Abbott', 'DHG Pharma', 'Hasan', 'Japanwell', 'Stella Pharm', 'Traphaco', 'OPV', 'Ha Noi Pharma', 'Vinh Phuc Pharma', 'Khanh Hoa Pharma', 'Sanofi', 'Bayer', 'Roche', 'Novartis'
];
...
const ProductList = () => {
  ...
  useEffect(() => {
    setPage(1);
  }, [searchTerm, filterCategory, filterCountry, filterBrand]);
  ...
  useEffect(() => {
    const filtered = products.filter(p => {
      ...
      const matchCountry = filterCountry === 'Tất cả' || p.country === filterCountry;
      const matchBrand = filterBrand === 'Tất cả' || p.brand === filterBrand;
      return matchSearch && matchCategory && matchCountry && matchBrand;
    });
    setFilteredProducts(filtered);
  }, [searchTerm, products, filterCategory, filterCountry, filterBrand]);
  ...
  return new Promise((resolve, reject) => {
    ...
    // Hàng 1: Logo (A1:B1) và Tên công ty căn giữa C1:F1
...
worksheet.getCell('P5').value = dateStr;
worksheet.mergeCells('P5:P6');
worksheet.getCell('P5').font = { italic: true };
...
worksheet.getRow(6).height = 22;
    ...
    // 3. Header bảng
    const headers = [
      'STT', 'Ảnh', 'Tên sản phẩm', 'Danh mục', 'Nước sản xuất', 'Thương hiệu', 'Giá niêm yết', 'Dạng bào chế', 'HSD (tháng)', 
    ...
    for (let i = 0; i < dataToExport.length; i++) {
      ...
      row.eachCell((cell, colNumber) => {
        ...
        // Căn giữa cho các cột: STT(1), Ảnh(2), Tên sản phẩm(3), Danh mục(4), Nước SX(5), Thương hiệu(6), Giá(7), Dạng bào chế(8), HSD(9)
    ...
    const colWidths = [10, 25, 45, 35, 20, 20, 18, 16, 12, 24, 24, 24, 24, 24, 18, 28];
    ...
    <div className="max-w-6xl mx-auto p-6">
      ...
      <div className="bg-white p-4 rounded-2xl border border-gray-100 shadow-sm mb-6 flex flex-col md:flex-row gap-4 items-center">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          ...
          <select
            className="flex-1 md:w-56 px-4 py-3 bg-gray-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-primary/20 outline-none"
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <select
            className="flex-1 md:w-56 px-4 py-3 bg-gray-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-cyan-500/20 outline-none"
            value={filterCountry}
            onChange={(e) => setFilterCountry(e.target.value)}
          >
            <option value="Tất cả">Tất cả nước SX</option>
        ...
        <div className="flex items-center gap-2 w-full md:w-auto">
          <select
            className="flex-1 md:w-56 px-4 py-3 bg-gray-50 border-none rounded-xl text-sm font-bold focus:ring-2 focus:ring-purple-500/20 outline-none"
            value={filterBrand}
        ...
        <div className="flex items-center gap-2 text-xs font-black text-gray-400 uppercase tracking-widest bg-gray-50 px-4 py-3 rounded-xl border border-gray-100">
          Hiển thị: <span className="text-primary">{visibleProductsList.length}</span> / {filteredProducts.length} sản phẩm
        </div>
      </div>
          ...
          <table className="min-w-full text-sm">
            <thead className="bg-gray-50 text-gray-600 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-left font-bold uppercase tracking-wider">Nước SX</th>
                <th className="px-4 py-3 text-left font-bold uppercase tracking-wider">Thương hiệu</th>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1fr)_360px] gap-10">
                <div className="space-y-6">
                  ...
                  <div className="grid md:grid-cols-2 gap-6">
                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                        <Globe size={18} className="text-cyan-500"/> Nước sản xuất
                      </label>
                      <select
                        className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-cyan-500 outline-none transition bg-gray-50/30"
                        value={form.country}
                    ...
                    <div>
                      <label className="block text-sm font-bold text-gray-700 mb-2 flex items-center gap-2">
                        <Building2 size={18} className="text-purple-500"/> Thương hiệu
                      </label>
                      <select
                        className="w-full px-4 py-3 border rounded-xl focus:ring-2 focus:ring-purple-500 outline-none transition bg-gray-50/30"
                        value={form.brand}
