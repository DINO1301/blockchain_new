...
import MomoLogo from '../../assets/momo_icon.png';
...
const Cart = () => {
  ...
  const isProcessingOrder = useRef(false); // Ref để chống lặp đơn hàng
  ...
  const [onlineProvider, setOnlineProvider] = useState('momo'); // 'momo', 'vnpay'

  // =====================================================================
  // 1. HÀM XỬ LÝ LƯU DATABASE CHUNG (Tái sử dụng cho cả Trực tiếp & Online)
  // =====================================================================
  const processAndSaveOrder = async (itemsToProcess, method, status, finalAmount) => {
    const orderDetails = []; 
    const productUpdates = [];

    // BƯỚC A: ĐỌC VÀ TÍNH TOÁN LÔ HÀNG (FIFO)
    ...
    // BƯỚC B: CẬP NHẬT TỒN KHO VÀO DB
    for (const update of productUpdates) {
      const { error: updateError } = await supabase
        .from('products')
        .update({ batches: update.batches, total_stock: update.total_stock })
        .eq('id', update.id);
      
      if (updateError) throw updateError;
    }

    // BƯỚC C: LƯU LỊCH SỬ ĐƠN HÀNG
    const { error: orderError } = await supabase.from('orders').insert([{
      user_id: user.id,
      user_email: user.email,
      user_full_name: user.full_name || user.email.split('@')[0],
  ...
  // =====================================================================
  // 2. USE EFFECT: GỘP & DỌN DẸP XỬ LÝ CALLBACK (Tránh trùng lặp code)
  // =====================================================================
  useEffect(() => {
    // A. Lắng nghe sự kiện back trang từ cache
    const handlePageShow = (event) => { if (event.persisted) setIsProcessing(false); };
    window.addEventListener('pageshow', handlePageShow);

    // B. Xử lý Callback từ MoMo hoặc VNPay
    const momoResult = searchParams.get('resultCode');
    const vnpayResult = searchParams.get('vnp_ResponseCode');

    if (momoResult !== null || vnpayResult !== null) {
      setIsProcessing(false);
      
      if (isProcessingOrder.current) return; // Chống lưu đơn hàng 2 lần
      isProcessingOrder.current = true;

      // Mã thành công (MoMo: 0, VNPay: 00) / Mã hủy (MoMo: 1006, VNPay: 24)
      const isSuccess = momoResult === '0' || vnpayResult === '00';
      const isCancel = momoResult === '1006' || momoResult === '1005' || vnpayResult === '24';
      
      if (isSuccess) {
        const savedData = localStorage.getItem('pending_online_order');
        const pendingData = savedData ? JSON.parse(savedData) : null;
        const itemsToProcess = cartItems.length > 0 ? cartItems : (pendingData?.items || []);
        
        if (itemsToProcess.length > 0) {
          const amountFromUrl = searchParams.get('amount') || searchParams.get('vnp_Amount');
          // Fix logic số tiền VNPay (VNPay tự nhân 100 trên URL, cần chia lại)
          const finalAmount = vnpayResult ? Number(amountFromUrl) / 100 : (amountFromUrl ? Number(amountFromUrl) : (pendingData?.totalAmount || totalAmount));
          const method = pendingData?.provider || 'online';

          // Gọi hàm lưu Database chung
          processAndSaveOrder(itemsToProcess, method, 'paid_online', finalAmount)
            .then(() => {
              setPaymentStatus({ type: 'success', message: `Thanh toán ${method.toUpperCase()} thành công!` });
              localStorage.removeItem('pending_online_order');
              clearCart();
        ...
        localStorage.removeItem('pending_online_order');
        setPaymentStatus({ type: 'cancel', message: 'Giao dịch đã bị hủy. Vui lòng thử lại.' });
      } else {
        localStorage.removeItem('pending_online_order');
        setPaymentStatus({ type: 'error', message: `Giao dịch thất bại (Mã lỗi: ${momoResult || vnpayResult}).` });
      }

      setSearchParams({}, { replace: true }); // Xóa param URL cho sạch
    }
  ...
  // =====================================================================
  // 3. LOGIC KHI BẤM NÚT "THANH TOÁN NGAY"
  // =====================================================================
  const handleCheckout = async () => {
    if (cartItems.length === 0) return;
    
    let methodText = paymentMethod === 'direct' ? "Trực tiếp tại quầy" : (onlineProvider === 'momo' ? "Ví điện tử MoMo" : "Cổng thanh toán VNPay");
    if (!window.confirm(`Xác nhận thanh toán đơn hàng trị giá ${totalAmount.toLocaleString()}đ?\nPhương thức: ${methodText}`)) return;

    setIsProcessing(true);
    setPaymentStatus(null); 

    // TRƯỜNG HỢP A: THANH TOÁN ONLINE (MOMO hoặc VNPAY)
    if (paymentMethod === 'online') {
      try {
        // Lưu tạm giỏ hàng phòng khi redirect về state bị reset
        localStorage.setItem('pending_online_order', JSON.stringify({
          items: cartItems,
          totalAmount: totalAmount,
          provider: onlineProvider, // 'momo' hoặc 'vnpay'
          timestamp: Date.now()
        }));
        
        // Gọi Edge Function động dựa trên biến onlineProvider
        const functionName = onlineProvider === 'momo' ? 'momo-payment' : 'vnpay-payment';
        const { data, error } = await supabase.functions.invoke(functionName, {
          body: { 
            amount: totalAmount, 
            orderInfo: `Thanh toan don hang MediTrack`,
            redirectUrl: window.location.href // Nếu Edge cần tự điều hướng về URL đang đứng
          }
        });
        
        if (error) throw error;
        
        // MoMo trả payUrl, VNPay trả paymentUrl (tùy cấu hình của bạn)
        const urlToRedirect = data?.payUrl || data?.paymentUrl; 
        
        if (urlToRedirect) {
          window.location.href = urlToRedirect; // Redirect trình duyệt sang cổng thanh toán
          return;
        } else {
          throw new Error(`Không nhận được link thanh toán từ ${onlineProvider.toUpperCase()}`);
        }
      } catch (err) {
        console.error(`Lỗi ${onlineProvider}:`, err);
        setPaymentStatus({ type: 'error', message: `Lỗi kết nối cổng thanh toán: ` + err.message });
        setIsProcessing(false);
        return;
      }
    }

    // TRƯỜNG HỢP B: THANH TOÁN TRỰC TIẾP
    try {
      // Tái sử dụng hàm lưu Database
      await processAndSaveOrder(cartItems, 'direct', 'pending', totalAmount);
  ...
  // =====================================================================
  // DƯỚI ĐÂY LÀ PHẦN RENDER GIAO DIỆN (GIỮ NGUYÊN)
  // =====================================================================
  if (cartItems.length === 0) {
    return (
      <div className="max-w-4xl mx-auto p-10 text-center">
        ...
        <Link to="/shop" className="bg-primary text-white px-6 py-3 rounded-lg font-bold hover:bg-blue-700 transition">
          Tiếp tục mua sắm
        </Link>
      </div>
    );
                ...
                <div className="grid grid-cols-1 gap-2">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <img src={MomoLogo} alt="momo_logo" className="w-10 h-10" />
                  ...
                  {/* VNPay */}
                  <button
                    onClick={() => setOnlineProvider('vnpay')}
                    className={`flex items-center justify-between p-4 rounded-2xl border-2 transition-all group ${
                  ...
                  >
                    <div className="flex items-center gap-3">
                      <img src={VNPayLogo} alt="vnpay_logo" className="w-10 h-10" />
                      <div className="text-left">
                        <p className={`text-sm font-black ${onlineProvider === 'vnpay' ? 'text-[#005BAA]' : 'text-gray-700'}`}>Ví điện tử VNPay</p>
                        <p className="text-[10px] text-gray-400 font-bold">Thanh toán an toàn qua VNPay</p>
                      </div>
                    </div>
                    <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                      onlineProvider === 'vnpay' ? 'border-[#005BAA] bg-[#005BAA]' : 'border-gray-200'
                    }`}>
                      {onlineProvider === 'vnpay' && <div className="w-2 h-2 bg-white rounded-full" />}
                    </div>
                  </button>
