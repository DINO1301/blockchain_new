...
const OrderHistory = () => {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      if (role !== 'admin' && role !== 'admin_on_chain') {
    ...
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between mb-8 gap-4">
        <h1 className="text-2xl font-bold flex items-center gap-2">
          <Package className="text-primary"/> {(role === 'admin' || role === 'admin_on_chain') ? 'Quản Lý Đơn Hàng' : 'Đơn Hàng Của Tôi'}
        </h1>
        
        <div className="flex items-center gap-4">
          {(role === 'admin' || role === 'admin_on_chain') && (
            ...
            <div key={order.id} className="bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-all duration-300">
              ...
              >
                <div className="flex items-center gap-5">
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap items-center gap-y-1 gap-x-4 mt-1.5">
                      ...
                      <span className="text-xs text-gray-400 font-bold flex items-center gap-1.5 flex-wrap">
                        ...
                        {order.user_full_name ? (
                          <span className="text-gray-600 font-medium">({order.user_full_name})</span>
                        ) : order.user_email ? (
                          <span className="text-gray-600 font-medium">({order.user_email.split('@')[0]})</span>
                        ) : null}
                      </span>
                    </div>
                  </div>
                ...
                <div className="flex items-center gap-8 mt-4 md:mt-0">
                  <div className="flex items-center gap-4">
                    {/* Nút xác nhận chỉ hiện cho ADMIN hoặc ADMIN_ON_CHAIN và khi đơn đang chờ thanh toán tiền mặt */}
                    {(role === 'admin' || role === 'admin_on_chain') && order.status === 'pending' && order.payment_method === 'direct' && (
                      <button 
