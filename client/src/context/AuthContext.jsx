...
export const AuthProvider = ({ children }) => {
  ...
  const handleUserSession = async (authUser) => {
    ...
    const roleTimeout = setTimeout(() => {
      ...
      setRole(authUser.email === "nguyenhieu@gmail.com" ? 'admin_on_chain' : 'user');
      setLoading(false);
    }, 5000);
    ...
    try {
      ...
      const ADMIN_ON_CHAIN_EMAIL = "nguyenhieu@gmail.com";
      const isHardcodedAdminOnChain = authUser.email === ADMIN_ON_CHAIN_EMAIL;
      
      if (!dbError && userData) {
        setUser({ ...authUser, ...userData });
        setRole(isHardcodedAdminOnChain ? 'admin_on_chain' : userData.role); 
      } else {
        console.warn("ℹ️ Không tìm thấy profile trong bảng users, dùng mặc định.");
        setRole(isHardcodedAdminOnChain ? 'admin_on_chain' : 'user');
      }
    } catch (error) {
